...
$pos = strpos($_POST['user'], 'C');
if($pos === false) {
    $_SESSION['rep_reg_msg'] =  "Client Reg CODE not valid, please try again.";
    print "<META http-equiv='refresh' content='0;URL=client-registration.php'>";	
   exit;
}
$c_date = date('Y-m-d');
...
$qry_code = "SELECT * FROM client WHERE cemail = '" . $_POST['email'] . "'";
$rs_code = $con->recordselect($qry_code);
$no_code = mysqli_num_rows($rs_code);
if($no_code > 0) 
{
...
$client_cid = "C510000001";
                
                $qry_check = "SELECT * FROM client order by c_id_key";
                $rs_check = $con->recordselect($qry_check);
                $no_clients = mysqli_num_rows($rs_check);
                
                if($no_clients>0)
                {
                    $client_cid = "C".(510000001+$no_clients);
                }
?>
    ...
    <div id="wrapper">
        <div id="header-wrapper">
            <header id="header" class="container">
            	<h1>Client Registration Confirmation</h1>
            </header>
        </div>
